<script lang="ts">
	...
	async function autoSizeFontSize(e: MouseEvent) {
		...
		const container = (e.currentTarget as HTMLElement).parentElement?.querySelector('.w-full.h-full') as HTMLElement;
		if (!container) return;
...
>
	...
	{#if isEditingText}
		<textarea
			...
			class="w-full h-full p-1 bg-white border-none outline-none resize-none text-black"
			style:writing-mode={block.vertical ? 'vertical-rl' : 'horizontal-tb'}
	...
	{#if isSelected}
		...
		<button
			...
			aria-label="Move box"
		>
			<ExpandOutline class="w-4 h-4 rotate-45" />
		</button>
