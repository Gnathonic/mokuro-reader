...
<div class="relative">
	...
	<Dropdown triggeredBy="#library-selector-btn" placement="bottom-end">
		{#if showSelector}
			...
			{#each libraryList as library}
				<DropdownItem
					...
					class="flex items-center gap-2 {selected === library.id ? 'bg-gray-100 dark:bg-gray-700' : ''}"
				>
					...
					{:else}
						<span class="w-4"></span>
					{/if}
					<span class="flex-1 truncate max-w-[200px]">{library.name}</span>
				</DropdownItem>
			{/each}
		...
		<DropdownItem onclick={goToLibraries} class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
