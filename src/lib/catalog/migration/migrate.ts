...
interface VolumeDataForMigration {
  ...
  // Note: thumbnails are NOT migrated - processThumbnails() regenerates them
  pages: Page[];
...
async function writeVolumeBatchToV3(
  ...
  await newDb.transaction('rw', [newDb.volumes, newDb.volume_ocr, newDb.volume_files], async () => {
    for (const { volumeData, pageCharCounts } of batch) {
      ...
      // Note: thumbnails are NOT migrated - they will be regenerated by processThumbnails()
      // This avoids migrating old algorithm thumbnails and prevents race conditions
...
async function migrateFromV2(
  ...
  try {
    ...
    for (const metadata of toMigrate) {
      ...
      const { thumbnail: _discardedThumbnail, ...metadataWithoutThumbnail } = metadata;
